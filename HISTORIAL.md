...
- **La actualización trabada por las excluidas (18/08)** — puse `config/excluidas.txt` en el repo para que "la lista te siga a las tres PCs", y con eso viajó también **la lista de bloqueadas del broker**. Dos errores en uno: es **distinta en cada broker** (Alpaca 863, Tastytrade 394, y no son los mismos, así que le habría excluido a Tasty símbolos que sí opera), y la app **la reescribe en cada refresco**, así que choca con el repo en cada actualización. `git pull` abortó en la PC de Tasty con *"untracked working tree files would be overwritten"*.
  - **Mi primer arreglo no alcanzó**, y esa es la parte que enseña: separé la del broker (a `.gitignore`) pero dejé la mía en el repo. El pull **siguió fallando igual**, porque el problema no era el contenido sino que el commit entrante **escribe en una ruta que en esa PC existe sin trackear**. Y el `Actualizar.bat` que lo resolvía venía *dentro* de la actualización trabada: no podía ayudarse a sí mismo.
  - **Lo que sí funcionó**: renombrar el archivo trackeado a `excluidas_compartidas.txt`. Como el commit entrante ya no toca la ruta conflictiva, el pull pasa **sin que el usuario haga nada**, y en la PC que ya lo tenía trackeado el contenido viaja al nombre nuevo sin perderse. Verificado antes de subirlo, con tres clones de prueba (PC sin el archivo / con el archivo sin trackear / con el archivo trackeado).
  - **La lección general: un archivo que la app reescribe sola no puede vivir en el repositorio.** Lo que viaja es una copia aparte que solo se toca a mano, y sirve de punto de partida cuando una PC todavía no tiene su lista.
- **El parpadeo del ladder (17/08)** — al mover una orden y al cancelar, la orden desaparecía un instante y volvía. Dos huecos distintos con la misma forma: **el dibujo provisorio se borraba antes de que el dato real estuviera**. Al mover, porque el hilo del ladder tiraba el id que devuelve `modify_order` y Alpaca/Tasty **reemplazan la orden por otra con otro id**: la pantalla esperaba un id que ya no iba a volver. Al cancelar, porque una lectura REST atrasada traía viva una orden que el aviso ya había dado por muerta. Arreglado pasando el id nuevo y dejando el dibujo puesto **2 segundos de gracia** después de que la orden desaparece: mientras existe, sus ids quedan tapados y la orden no puede revivir ni dibujarse dos veces. **Era solo visual: no mandaba ni cancelaba nada de más.**
