...
class LadderWorker(QObject):
    ...
    @Slot(object, str, int, float, bool)
    def mandar(self, side, symbol: str, qty: int, precio: float, extended: bool) -> None:
        broker = self._broker()
        if broker is None:
    ...
    @Slot(object, float)
    def mover(self, pares, nuevo: float) -> None:
